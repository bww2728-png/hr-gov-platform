...
| الميزة | الحالة |
|---|---|
| التكاملات الحكومية (Qiwa / Muqeem / GOSI / Absher) | **قيد التطوير** — سجلات وحالات داخلية فقط، لا يوجد اتصال فعلي بالمنصات الخارجية بعد |
| MFA | **قيد التطوير** — الحماية الحالية: سياسة كلمات مرور قوية (12+ حرفاً مع تعقيد)، قفل حسابات، rate limit للدخول |
| SSO مؤسسي | **قيد التطوير** |
| حسابات تجريبية | تُعطَّل في الإنتاج عبر `POST /api/admin/bootstrap/production-hardening` (sysadmin + BOOTSTRAP_TOKEN) |
| دفتر حركات الرواتب (Ledger) | منفذ — `payroll_ledger_entries` + `GET /api/payroll/runs/:id/ledger` |
| إخفاء البيانات الحساسة | منفذ — في الموظفين وكل التقارير وطرق العرض T1..T12 |
| Migrations versioned | منفذ — `prisma/migrations` + `migrate deploy` عند الإقلاع (fallback غير مدمّر لقواعد سابقة) |
| توثيق الخصوصية | `docs/PRIVACY_PDPL.md` |
| نتائج الأداء | `docs/PERFORMANCE.md` |
